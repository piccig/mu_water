...
namespace PLMD{
  namespace colvar{
    ...
    */
    ...
    class mudelhist : public Colvar {
      //      SwitchingFunction switchingFunction; //instance sw.f
      int Natoms, id, Ndel;
      double sigma_LJ, eps_LJ, R_min, R_max, r_cut, r_skin, coff, kT, r0;
      ...
      //histogram variables
      bool rwhist, nofirst, dufile;
      int Nbin, Nsmall_av, Nlarge_av, Nhist_av,histprint;
      double Wmax, Ubin, Ztot;
    ...
    PLUMED_REGISTER_ACTION(mudelhist,"MUDELHIST")

    void mudelhist::registerKeywords(Keywords& keys){
      ...
      keys.addFlag("DUFILE",false,"Plot Deletion Energy File");
      keys.remove("NOPBC");
    }
    
    mudelhist::mudelhist(const ActionOptions&ao):
	...
	fhist<< endl;
	dhist.open("deldata.dat"); //file containing the characteristic numbers for the collected histogram
	//header dhist
	dhist << "ljlist.step\tNsmall\t<Nsmall>\tNlarge\t<Nlarge>\tNhist\t<Nhist>\tNtot\t" << endl;
    ...
    double mudelhist::swfon(double z, double Coff){
      double sig;
      if( z < -Coff){
	sig=0.0;
      }else if(z > Coff){
    ...
    double mudelhist::swfoff(double z, double Coff){
      double sig;
      if( z < -Coff){
	sig=0.0;
      }else if(z > Coff){
    ...
    double mudelhist::dswf(double z, double Coff){
      double dsig;
      if(fabs(z) > Coff){
	dsig=0.0;
      }else{
    ...
    //LJ Potential
    double mudelhist::phi(double rq, double rmin, double rmax, double sigma, double eps, double Vmax, double Vshift){
    ...
    //LJ derivative
    double mudelhist::dphi(double rq, double r, double rmin, double rmax, double sigma, double eps){
    ...
    //LJ Potential
    double mudelhist::phis(double rq, double r, double rmin, double rmax, double sigma, double eps, double Vmax, double Vshift, double r0, bool issig, double coff){
    ...
    //LJ derivative
    double mudelhist::dphis(double rq, double r, double rmin, double rmax, double sigma, double eps, double Vmax, double Vshift, double r0, bool issig, double coff){
    ...
    double mudelhist::phiq(double rq, double r, double rmin, double rmax, double sigma, double eps, double Vmax, double Vshift, double dmin){
      double phi, sorq, sor6, sor12;
      if(rq <= rmin*rmin){
    ...
    //LJ derivative
    double mudelhist::dphiq(double rq, double r, double rmin, double rmax, double sigma, double eps, double dmin){
      double dphi, sorq, sor6, sor12;
      if( rq <= rmin*rmin){
	dphi=r*dmin/rmin;
      }else if(rq >= rmax*rmax){
    ...
    void mudelhist::lj_newlist(vector<Vector> &del, ljlist_s &ljlist)
    {
      int i,j;
      Vector rij, test;
      double mod_rij;
      ...
      //fdbg << "N del: " << ndel << endl;
      //fdbg << "listsize: " << listsize << endl;
      //fdbg.flush();
      

      //fill(ljlist.nn.begin(), ljlist.nn.end(), 0);  //zero neighbor numbers


      //fdbg << "nnsize: " << ljlist.nn.size() << endl;
      //fdbg.flush();
    ...
    void mudelhist::lj_checklist(vector<Vector> &del, ljlist_s &ljlist)
    {
      unsigned int j; 
	...
	if(ljlist.step==-1){ //first step
      ...
      ++ljlist.step; //0 first step, or -1 if nofirst
	...
	if(ljlist.step>=0){
	  ...
	  //Fill deletion energy histogram
	  if(histp[2]>0.0){
	    double bDu = beta*Du;
	    if(bDu<histp[0]){
	      Nsmall++;
	...
	vector<Vector>().swap(phiprime);
      ...
      if(dufile && ljlist.step>=0){
	comm.Sum(bDuvec);
	if(rank == 0){
	  for(int j=0 ; j<Ndel; j++){ 
	    fdu << setprecision(8);
	    fdu << (ljlist.step)*Ndel+j << "\t" << bDuvec[j] << "\t" << ljlist.step << endl;
	  }
	}
      }
      ...
      if(Su==0){
	setValue(-kT*coff); //When the integral is 0 mu=coff* kT (very unlikely!!!)
      }else{
	...
	weight=exp(beta*(bias-coft-Wmax));
      ...
      //DELETION ENERGY HISTOGRAM: print and update
      
      if(histp[2]>0.0 && ljlist.step>=0){
	...
	//print on files
	if((ljlist.step+1)%histprint == 0 && rank == 0){
	  ofstream favhist;
	  favhist.open("avpdUdel.dat"); //file t-averaged histogram
	  fhist << setprecision(5);
	  ...
	  dhist << ljlist.step << "\t" << Nsmall << "\t"  << 1.0*Nsmall_av/(ljlist.step+1) << "\t"<< Nlarge << "\t"  << 1.0*Nlarge_av/(ljlist.step+1)<< "\t" << Nhist << "\t"  << 1.0*Nhist_av/(ljlist.step+1) << "\t" << Ntot << "\t" << fixed << setprecision(8) << weight << endl; //hist N data
	  fhist.flush();
	  dhist.flush();
	  favhist.close();
