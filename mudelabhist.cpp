...
namespace PLMD{
  namespace colvar{
    ...
    */
    ...
    class mudelabhist : public Colvar {
    ...
    public:
      ...
      mudelabhist(const ActionOptions&);
    ...
    PLUMED_REGISTER_ACTION(mudelabhist,"MUDELABHIST")

    void mudelabhist::registerKeywords(Keywords& keys){
    ...
    mudelabhist::mudelabhist(const ActionOptions&ao):
    ...
    double mudelabhist::swfon(double z, double Coff){
      double sig;
      if( z < -Coff){
	sig=0.0;
      }else if(z > Coff){
    ...
    double mudelabhist::swfoff(double z, double Coff){
      double sig;
      if( z < -Coff){
	sig=0.0;
      }else if(z > Coff){
    ...
    double mudelabhist::dswf(double z, double Coff){
      double dsig;
      if(fabs(z) > Coff){
	dsig=0.0;
      }else{
    ...
    //LJ Potential
    double mudelabhist::phi(double rq, double rmin, double rmax, double sigma, double eps, double Vmax, double Vshift){
    ...
    //LJ derivative
    double mudelabhist::dphi(double rq, double r, double rmin, double rmax, double sigma, double eps){
    ...
    //LJ Potential
    double mudelabhist::phis(double rq, double r, double rmin, double rmax, double sigma, double eps, double Vmax, double Vshift, double r0, bool issig, double coff){
    ...
    //LJ derivative
    double mudelabhist::dphis(double rq, double r, double rmin, double rmax, double sigma, double eps, double Vmax, double Vshift, double r0, bool issig, double coff){
    ...
    double mudelabhist::phiq(double rq, double r, double rmin, double rmax, double sigma, double eps, double Vmax, double Vshift, double dmin){
      double phi, sorq, sor6, sor12;
      if(rq <= rmin*rmin){
    ...
    //LJ derivative
    double mudelabhist::dphiq(double rq, double r, double rmin, double rmax, double sigma, double eps, double dmin){
      double dphi, sorq, sor6, sor12;
      if( rq <= rmin*rmin){
	dphi=r*dmin/rmin;
      }else if(rq >= rmax*rmax){
    ...
    void mudelabhist::lj_newlist(vector<Vector> &del, ljlist_s &ljlist)
    {
      int i,j;
      Vector rij, test;
      double mod_rij;
    ...
    void mudelabhist::lj_checklist(vector<Vector> &del, ljlist_s &ljlist)
    {
      unsigned int j; 
